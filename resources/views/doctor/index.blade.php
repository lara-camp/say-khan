...
@section('content')
    ...
    <form action="{{ route('user.logout') }}" method="post">
        @csrf
        <button class="btn btn-danger my-3">Logout</button>
    </form>
    {{-- table --}}
                    ...
                    <tr>
                        ...
                        <td class="align-items-center">
                            <div class="btn-group" doctor="group">
                                <a href="{{ route('doctor.edit', encrypt($doctor->id)) }}" type="button"
                                    class="btn btn-danger">Edit {{ $doctor->id }}</a>
                                <a href="{{ route('feedback_create', encrypt($doctor->id)) }}" type="button"
                                    class="btn btn-danger">Give Feedback</a>
                                <a href="{{ route('clinic_subscription_create', encrypt($doctor->id)) }}" type="button"
                                    class="btn btn-danger">Buy Subscription</a>
